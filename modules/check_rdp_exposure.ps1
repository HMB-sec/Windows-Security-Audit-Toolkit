...
if ($rdpStatus.fDenyTSConnections -eq 0) {
    Write-Output "INFO: RDP is ENABLED"
} else {
    Write-Output "PASS: RDP is DISABLED"
}
...
if ($rdpPortCheck) {
    Write-Output "WARNING: RDP Port 3389 is LISTENING"
} else {
    Write-Output "PASS: RDP Port 3389 is NOT LISTENING"
}
...
if ($rdpFwRule) {
    Write-Output "WARNING: Windows Firewall has RDP RULE ENABLED"
} else {
    Write-Output "PASS: No enabled RDP firewall rule detected"
}
